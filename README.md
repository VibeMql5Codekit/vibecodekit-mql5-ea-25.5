...
[![version](https://img.shields.io/badge/version-v1.6.0-blue)](https://github.com/BuildMqlCodekit-01/vibecodekit-mql5-ea/releases/tag/v1.6.0)
[![tests](https://img.shields.io/badge/tests-1448%20passing-success)]()
...
| Thành phần | Đã giao |
|-----------|---------|
| **Lệnh CLI** | 68 lệnh (Wave 3 cứng 10 lệnh review/RRI về 2 umbrella: `mql5-review --lens {eng,ceo,cso,investigate}` và `mql5-rri {template,bt,rr,chart}`; 10 console-script cũ vẫn chạy như alias) — đầy đủ chu trình `scan → plan → build → verify → review → deploy → ship`, cộng `mql5-init` wizard, `mql5-auto-build --draft`, `mql5-auto-fix`, `mql5-spec-from-prompt`, `mql5-fixture` (hermetic XML, không cần Wine), `mql5-forge-loop` (Wave-3.A/B/C), `mql5-bt-sim` (Wave-3.E: backtest tick-bar in-process bằng Python thuần, emit XML đúng schema MT5), bộ sinh step-output Wave 5.1 (`mql5-vision-gen` / `mql5-blueprint-gen` / `mql5-tip-gen`), bộ governance emitter Wave 6.1 (`mql5-contract-gen` / `mql5-verify-report` / `mql5-permission-layer5 --enforce-sign-off`), bộ task-graph / completion emitter Wave 6.2 (`mql5-task-graph-gen` / `mql5-completion-report`), `mql5-manifest`, `mql5-dashboard`. |
...
| **Test gate** | 1448 test pass qua Phase 0/A/B/C/D/E (Wave 1 + Wave 2 + Wave 3 + Wave 4.3 audit cell-coverage + Wave 5.1 step-output generators + Wave 5.2 sentinel-content validator + Wave 5.3 persona prompts + Wave 6.1 actor prompts + contract-gen + verify-report + sign-off audit + Wave 6.2 task-graph-gen + completion-report, kèm golden dataset 20 EA ở `tests/fixtures/ea-bugs/`). |
