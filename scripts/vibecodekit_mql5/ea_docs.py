...
from .ea_docs_markdown import render_markdown
...
# The Markdown renderer was split out to :mod:`ea_docs_markdown` to
# keep this module under the audit script's per-module LOC ceiling.
# ``render_markdown`` is re-exported via the top-of-file import so
# existing callers (and ``__all__``) keep working unchanged.
